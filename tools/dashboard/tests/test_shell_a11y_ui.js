...
function assertM2Scenario(name, viewport) {
  if (name === 'workspace') {
    ...
    '}');
    // Master requires process verification: saved activity alone is a stopped issue.
    assert.deepEqual(workspace.filters, ['Waiting on you 3', 'In progress 1', 'Paused / issues 3', 'Completed 1'], viewport.name + ' Workspace preserves the four exact aggregate filters');
    assert.deepEqual(workspace.filterNames, ['Waiting on you 3', 'In progress 1', 'Paused / issues 3', 'Completed 1']);
    ...
    assert.deepEqual(workspace.rows.map(row => row.priority), [...workspace.rows.map(row => row.priority)].sort((a, b) => a - b), viewport.name + ' Workspace rows follow attention, stopped issues, live work, and completion priority');
