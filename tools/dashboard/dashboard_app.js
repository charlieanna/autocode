...
function monitorPanel(run, compact=false) {
  ...
  if(!compact){
    ...
    history.append(n('h3','Open findings'),Object.assign(n('p',(ledger?ledger.open+' open · '+ledger.resolved+' resolved'+(ledger.repeated?' · '+ledger.repeated+' reported again after a fix':'')+'. Both reviewers, one list; a finding closes only when the reviewer who raised it no longer reports it.':'Latest saved review'+(monitor.validation_verdict?' · '+monitor.validation_verdict:'')+'. Fixes in progress may not yet be revalidated.')),{className:'monitor-caption'}));
    for(const finding of monitor.findings||[]){const row=card('','monitor-event');row.append(n('span',(finding.id?finding.id+' · ':'')+(finding.finding||'Finding details unavailable')),n('small',human(finding.severity)+(finding.source?' · raised by '+human(finding.source):'')+(finding.assigned_task?' · fix: '+finding.assigned_task:'')+((finding.times_reported||1)>1?' · reported '+finding.times_reported+'×':'')));history.append(row);}
